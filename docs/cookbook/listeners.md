[Event Listeners](https://athena-framework.github.io/athena/Athena/EventDispatcher/EventListenerInterface.html) are a core part of Athena's architecture that allows tapping into specific events within the life-cycle of each request.  Custom events can also be defined, dispatched, and listened upon.  See the related [EventDispatcher](https://athena-framework.github.io/athena/Athena/EventDispatcher.html) component for more information.

## JWT Security

Currently Athena does not have any built in abstractions related to authentication or authorization.  This feature is planned and will be implemented at some point in the future.  Until then however, we can define a security listener that implements our authentication logic via listening on the [ART::Events::Request](https://athena-framework.github.io/athena/Athena/Routing/Events/Request.html) event which includes a reference to the original `HTTP::Request` object.
...
From here, it would be up to the developer to implement a way to authorize the user now that they have been authenticated and are accessible within the application.  One option could be to utilize [Configuration Annotations](https://athena-framework.github.io/athena/Athena/Config.html#configuration_annotation(name,*args,&)-macro) as a means to "tag" controller actions with specific "levels" of security; then add another `#call` method to the security listener to listen on the [ART::Events::Action](https://athena-framework.github.io/athena/Athena/Routing/Events/Action.html) event which exposes the [ART::Action](https://athena-framework.github.io/athena/Athena/Routing/Action.html) related to the current request from which the annotations could be read off of.

> **NOTE:** This example is a modified version of the one used as part of the [JSON API Blog Tutorial](https://dev.to/blacksmoke16/creating-a-json-api-with-athena--granite-510i) blog post.

## Pagination

Generic pagination can be implemented via listening on the [ART::Events::View](https://athena-framework.github.io/athena/Athena/Routing/Events/View.html) event which exposes the value returned via the related controller action.  We can then define a `Paginated` [Configuration Annotation](https://athena-framework.github.io/athena/Athena/Config.html#configuration_annotation(name,*args,&)-macro) that can be applied to controller actions to have them be paginated via the listener.
